...
def update_object(
    paths: ProjectPaths, *, object_id: str, rating: Rating | None = None
) -> dict[str, Any]:
    repo = Repository(paths)
...
def ensure_report(paths: ProjectPaths, *, object_id: str) -> dict[str, Any]:
    ...
    if not report_path.exists():
        ...
        report_path.write_text(_report_stub(research_object), encoding="utf-8")
...
def _report_stub(research_object: ResearchObject) -> str:
    title = research_object.title or title_from_id(research_object.object_id)
    return (
        "---\n"
        f"object_id: {research_object.object_id}\n"
        f"title: {yaml.safe_dump(title, default_flow_style=True).strip()}\n"
        "rating: null\n"
        "status: pending\n"
        "---\n\n"
...
def ensure_note(paths: ProjectPaths, *, object_id: str) -> dict[str, Any]:
    ...
    if not note_path.exists():
        note_path.write_text(_note_stub(research_object), encoding="utf-8")
...
def _note_stub(research_object: ResearchObject) -> str:
    title = research_object.title or title_from_id(research_object.object_id)
    tag_block = (
        "tags:\n" + "\n".join(f"  - {tag}" for tag in research_object.tags)
        if research_object.tags
        else "tags: []"
    )
    return (
        "---\n"
        f"object_id: {research_object.object_id}\n"
        f"title: {yaml.safe_dump(title, default_flow_style=True).strip()}\n"
        f"rating: {research_object.rating}\n"
        f"report: '[[Reports/{research_object.object_id}]]'\n"
        f"{tag_block}\n"
        "---\n\n"
